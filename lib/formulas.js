...
// ── DIFERENCIA DE DÍAS (INCLUSIVA) ───────────────────────────
// Toda fecha representa un día completo: de 00:00:00 a 23:59:59.
// Por eso la diferencia entre dos fechas cuenta AMBOS extremos:
//   del 01/06 al 12/06 → 12 días (no 11), porque cada día —incluido
//   el de inicio y el de fin— es una jornada completa disponible.
// Esta es la ÚNICA función que debe usarse para restar fechas en
// todo el sistema, para que el criterio sea siempre el mismo.
function diferenciaDiasInclusiva(fechaA, fechaB) {
  // fechaB - fechaA, ambas Date a medianoche. +1 para incluir el día de inicio.
  return Math.round((fechaB - fechaA) / 86400000) + 1
}
...
export function getDiasPlazo(fecha_inicio, fecha_limite) {
  ...
  return diferenciaDiasInclusiva(ini, lim)
}
...
export function getEstado(fecha_inicio, fecha_limite, fecha_reporte) {
  ...
  if (hoy > lim) return 5
  const diasRestantes = diferenciaDiasInclusiva(hoy, lim)
  // diasRestantes=1 significa "vence hoy" (mismo día = 1 día completo inclusive)
  return diasRestantes <= 4 ? 4 : 3
}
...
export function getDuracionReal(fecha_inicio, fecha_limite, fecha_reporte) {
  ...
  const ini = new Date(fecha_inicio + 'T00:00:00')
  if (fecha_reporte) {
    return diferenciaDiasInclusiva(ini, new Date(fecha_reporte + 'T00:00:00'))
  }
  if (!fecha_limite) return null
  const lim = new Date(fecha_limite + 'T00:00:00')
  const hoy = new Date(); hoy.setHours(0, 0, 0, 0)
  if (hoy > lim) return diferenciaDiasInclusiva(ini, hoy)
  return null
}
...
export function getDiasRestantes(fecha_limite) {
  ...
  const lim = new Date(fecha_limite + 'T00:00:00')
  const hoy = new Date(); hoy.setHours(0, 0, 0, 0)
  return diferenciaDiasInclusiva(hoy, lim)
}
...
// ── CÓDIGO OT ─────────────────────────────────────────────────
// Formato estándar : EPU{semana}{semestre}P{año}  → EPU16IP26
// Formato NTCSE    : EPUA{mes}-{R|U}{semana}       → EPUA07-R02
export function generarCodigoOT(semana, periodo = '2026-I', opts = {}) {
  if (!semana) return ''
  const mx = (opts.motivo || '').toUpperCase().trim()
  const numSemana = (String(semana).match(/\d+/) || ['00'])[0].padStart(2, '0')
  if (mx === 'NTCSE RURAL' || mx === 'NTCSE URBANO') {
    const fi = opts.fechaInicio || ''
    const mes = fi
      ? String(new Date(fi.slice(0, 10) + 'T12:00:00').getMonth() + 1).padStart(2, '0')
      : '00'
    const letra = mx === 'NTCSE URBANO' ? 'U' : 'R'
    return `EPUA${mes}-${letra}${numSemana}`
  }
...
export function getEficienciaOperativa(ot) {
  ...
  if (fecha_inicio) {
    const ini = new Date(fecha_inicio + 'T00:00:00')
    const durReal = Math.max(1, diferenciaDiasInclusiva(ini, rep))
    const plazo   = dias_plazo || Math.max(1, diferenciaDiasInclusiva(ini, lim))
